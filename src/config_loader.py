...
class Config:
    ...
    def _validate(self):
        """Verifica la presenza delle sezioni essenziali"""
    ...
    def _setup_reproducibility(self):
        """Setta il seed per riproducibilità"""
    ...
    def get(self, key: str, default=None) -> Any:
        """
        Permette l'accesso alle informazioni presenti nello YAML, con possibilità di
        utilizzare la notazione dot: config.get('paths.video_dir')
        """
        keys = key.split('.')
        value = self._config
    ...
    def update(self, updates: Dict):
        """Aggiorna il config con i valori in args"""
    ...
    def save(self, save_path: str):
        """Salva config in un nuovo file"""
        with open(save_path, 'w') as f:
            yaml.dump(self._config, f, default_flow_style=False, sort_keys=False)
        print(f"Config salvato in: {save_path}")

    def create_directories(self):
        """Crea tutte le cartelle elencate in paths"""
...
def load_config(config_path: str = "config/config.yaml") -> Config:
    """Helper function per caricare il config"""
