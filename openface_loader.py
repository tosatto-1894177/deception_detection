...
def test_loader():
    ...
    print("\n" + "=" * 80)
    print("TEST OPENFACE LOADER")
    print("=" * 80)

    # Crea loader
    loader = OpenFaceLoader(
        csv_dir="data/openface",
        use_aus=True,
        use_gaze=True,
        use_pose=True,
        normalize=True
    ...
    # Test su una clip
    clip_name = "AN_WILTY_EP15_lie4"
    print(f"\nTesting su clip: {clip_name}")

    features = loader.load_and_preprocess(clip_name)

    if features is not None:
        print(f"\n✅ Features caricate con successo!")
        ...
        # Mostra feature names
        print(f"\n📋 Nomi delle prime 10 feature su {len(loader.get_feature_names())}:")
        for i, name in enumerate(loader.get_feature_names()[:10]):
            print(f"   {i}: {name}")
    else:
        print(f"❌ Errore caricamento features")
